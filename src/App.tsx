...
import { NotificationOnboarding } from './components/dialogs/NotificationOnboarding';

const NOTIF_PROMPT_SEEN_KEY = 'am_notif_prompt_seen';
...
function Shell() {
  ...
  /* One-time "enable reminders" prompt on first launch — only if the user
     hasn't already decided (granted/denied) and hasn't dismissed it before. */
  useEffect(() => {
    const alreadySeen = localStorage.getItem(NOTIF_PROMPT_SEEN_KEY);
    const supported    = isAndroid || 'Notification' in window;
    if (!alreadySeen && supported && getNotificationPermission() === 'default') {
  ...
  async function handleEnableNotifications() {
    ...
    if (perm === 'granted') {
      useSettingsStore.getState().update({ notificationsEnabled: true });
      ...
      toast.success("You're all set! We'll remind you before deadlines.");
    } else if (perm === 'denied') {
      toast.warning('No worries — you can turn reminders on anytime in Settings.');
    }
  ...
  return (
    ...
    >
      ...
      {/* ── First-launch "enable reminders" prompt ───────── */}
