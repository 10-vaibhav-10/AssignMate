...
export default function AssignmentDetail() {
  ...
  /* Study plan is derived live from real tasks (grouped by due date), not a
     separate AI-generated blob — so it can never drift out of sync with the
     checklist as tasks are added, edited, completed, or deleted. */
  const studyPlan: StudyPlanDay[] = Object.values(
    assignmentTasks
      .filter((t) => t.dueDate)
      .reduce<Record<string, StudyPlanDay>>((acc, t) => {
        const date = t.dueDate!;
        (acc[date] ??= { date, items: [] }).items.push({ title: t.title, completed: t.completed });
        return acc;
      }, {}),
                        ...
                        <div key={day.date}>
                          ...
                          <ul className="space-y-1">
                            {day.items.map((item, i) => (
                              ...
                              >
                                <span className={`mt-0.5 ${item.completed ? 'text-emerald-400' : 'text-violet-400'}`}>
                                  {item.completed ? '✓' : '•'}
                                </span>
                                {item.title}
                              </li>
