 ...
 *   3. GEMINI_API_KEY_1 … _N      → Google Gemini (gemini-3.5-flash, 250k TPM free)
 *   4. GROQ_API_KEY_1 … _N        → Groq fallback (12k TPM free, 1,000 req/day cap per key)
 */
function aiProxy(
  provider: {
    url: string;
...
export default defineConfig(({ mode }) => {
  ...
  if (openrouterKeys.length > 0) {
    ...
    console.log(`[ai-proxy] Using Gemini — ${geminiKeys.length} key${geminiKeys.length > 1 ? 's' : ''} (gemini-3.5-flash, 250k TPM free tier)`);
    provider = { url: GEMINI_URL, keys: geminiKeys, modelMap: GEMINI_MODEL_MAP, fallback: 'gemini-3.5-flash' };
  } else if (groqKeys.length > 0) {
    console.log(`[ai-proxy] Using Groq — ${groqKeys.length} key${groqKeys.length > 1 ? 's' : ''} (12k TPM free tier, 1,000 req/day cap)`);
